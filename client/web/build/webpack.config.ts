...
const config: Configuration = {
  ...
  module: {
    rules: [
      ...
      // Plain .css must NOT pass through less-loader: less resolves
      // `@import 'tailwindcss'` itself and inlines the package's JS entry.
      // The shared loader tail (css/postcss) is identical for both rules.
      {
        test: /\.css$/,
        use: [
          {
          ...
          },
          {
            ...
            options: {
              ...
              // let @tailwindcss/postcss handle @import resolution
              import: false,
            },
          },
          {
            ...
            options: {
              postcssOptions: {
                // Inline rather than postcss.config.js: mini-star plugin
                // builds also resolve that file and must not pull in the
                // Tailwind pipeline (plugin classes are generated by the main
                // app stylesheet via tailwind.config.js content globs).
                config: false,
